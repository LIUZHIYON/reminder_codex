...
| 项目 | 值 |
|------|-----|
| 连接地址 | `ws://{host}:9099/api/v1/aipet/ws/{serial_number}` |
| 认证端点 | `GET /api/v1/aipet/ws/auth/{serial_number}`（HTTP，获取连接令牌） |
| 消息格式 | JSON |
| 编码 | UTF-8 |
...
3. 如果提供了 `command_id`，更新 MySQL `ai_pet_command_logs` 表
4. 如果是 `reminder` 指令，同时更新 `ai_pet_reminders` 表
5. 如果是 `relay` 指令，同时更新 `ai_pet_relay_messages` 表
...
设备通过此消息查询服务端业务数据。支持 22 种 `request_type`。
...
| `get_relay_messages` | 查询传话消息 | page_num, page_size, content_type, status |
...
| `app_request` | 设备→服务器 | 是 | 业务请求（22 种子类型） |
...
Device                                    Server
  |                                          |
  |<--- server_command ----------------------|  命令下发（status=sent）
  |     {type:"server_command",              |  写入 Redis: aipet_command_pending:{cmd_id}
  |      command:"reminder",                 |  写入 MySQL: ai_pet_command_logs
  |      command_params:{...}}               |
  |                                          |
  |  设备收到，开始执行                       |
  |                                          |
  |--- command_response -------------------->|  设备回报执行中（可选）
  |     {type:"command_response",            |  更新 MySQL status=executing
  |      status:"executing"}                 |
  |                                          |
  |  设备执行中...                            |
  |                                          |
  |--- command_response -------------------->|  设备回报最终结果
  |     {type:"command_response",            |  Redis: lpush aipet_command_history:{id}
  |      status:"success",                   |  MySQL: update ai_pet_command_logs
  |      result:{...}}                       |  MySQL: update ai_pet_reminders (reminder)
  |                                          |  MySQL: update ai_pet_relay_messages (relay)
  |                                          |
```
...
| `module_aipet/websocket/handlers/command_handler.py` | 指令响应处理 |
...
| `module_aipet/websocket/handlers/app_request_handler.py` | 业务请求处理（22 子类型） |
| `module_aipet/websocket/handlers/base_handler.py` | 处理器基类 |
| `module_aipet/service/ai_pet_websocket_service.py` | 服务器→设备消息发送 |
| `module_aipet/config/enums.py` | 所有枚举定义 |
