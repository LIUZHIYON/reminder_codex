...
| 值 | 说明 | command_params |
|---|------|------|
| reminder | 待办事提醒 | `reminder_data`（标题/内容/时间/重复类型） |
| relay | 传话 | `content_type`、`content`/`media_url`、`relay_from`、`relay_to` |
...
| commandId | string? | 下发到设备的 command_id |
| commandParams | dict? | 下发设备的完整指令参数 |
...
**说明:** `petId`、`userId`、`createBy` 由服务端自动填充，无需客户端传入。新增后 status 默认为 pending。
...
```
POST /api/v1/aipet/app/reminders/send/{aipet_id}/{reminder_id}
```

**说明:** 将已创建的提醒通过 WebSocket/Redis 下发到设备。成功后自动更新 `status=sent`、`command_id`、`command_params`、`sent_time`。

**下发流程：**
1. 校验用户登录和宠物归属
2. 查询提醒记录，构建 `command_params`（含 `reminder_data` 和 `reminder_source: "app_chat"`）
3. 调用 `send_command_to_aipet()` 下发指令（内部统一写入 `ai_pet_command_logs`）
4. 成功则更新 `ai_pet_reminders` 表状态
...
```json
{
  ...
  "msg": "提醒已下发到设备",
  "data": {
    "command_id": "cmd_1_1718800000_abc123",
    "success": true
  }
}
```
...
**说明:** `petId`、`userId`、`createBy` 由服务端自动填充。新增时自动校验内容格式。
...
```
POST /api/v1/aipet/app/relayMessages/send/{aipet_id}/{relay_id}
```

**说明:** 将已创建的传话消息通过 WebSocket/Redis 下发到设备。成功后自动更新 `status=sent`、`command_id`、`command_params`、`sent_time`。

**下发流程：**
1. 校验用户登录和宠物归属
2. 查询传话记录，根据 content_type 构建 `command_params`（text 模式含 content/voice_style/speed；voice 模式含 media_url/media_duration/media_size）
3. 调用 `send_command_to_aipet()` 下发指令（内部统一写入 `ai_pet_command_logs`）
4. 成功则更新 `ai_pet_relay_messages` 表状态
...
```json
{
  ...
  "msg": "传话已下发到设备",
  "data": {
    "command_id": "cmd_1_1718800000_def456",
    "success": true
  }
}
```
