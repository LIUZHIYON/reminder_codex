...
**原因：** `_board_loop()` 在主协程中同步调用 `process_reminders()`，后者触发 PowerShell TTS 合成时 `subprocess.run` 设置了 30 秒超时。PowerShell 初始化卡住时，整个事件循环被阻塞 30 秒，导致定时器无法正常轮询后续提醒。

**修复：**
- `_board_loop()` 改用 `loop.run_in_executor(None, ...)` 在独立线程中运行提醒检查，不阻塞事件循环
- PowerShell TTS 超时从 30 秒降至 10 秒，快速回退到下一个 TTS 后端
