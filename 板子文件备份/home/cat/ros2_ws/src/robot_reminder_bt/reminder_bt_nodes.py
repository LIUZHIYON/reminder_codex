...
class GenerateTTS(AsyncActionNode):
    ...
    def _run(self, text):
        try:
            safe = text.replace(chr(34), chr(39))
            import subprocess as _sp, os as _os
            cmd = ["ros2", "action", "send_goal", "/voice/speak", "robot_voice_bridge/action/Speak", "{text: " + chr(39) + safe + chr(39) + "}"]
            env = _os.environ.copy()
            r = _sp.run(cmd, env=env, capture_output=True, timeout=20)
            self._ok = (r.returncode == 0)
            if not self._ok:
                err = (r.stdout + r.stderr).decode("utf-8", errors="replace")[:200]
                print(f"[BT-TTS] action failed: {err}")
