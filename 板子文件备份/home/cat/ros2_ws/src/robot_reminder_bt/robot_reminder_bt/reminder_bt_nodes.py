...
class GenerateTTS(AsyncActionNode):
    # publish text to /tts/text topic for voice_bridge playback
    ...
    def _run(self, text):
        try:
            ...
            Q = chr(34)
            scr = "#!/bin/bash" + NL
            scr += "source /opt/ros/humble/setup.bash" + NL
            scr += "source ~/ros2_ws/install/setup.bash 2>/dev/null" + NL
            scr += "timeout " + str(timeout_sec) + " ros2 action send_goal /voice/speak robot_voice_bridge/action/Speak '" + "{" + "text: " + Q + safe + Q + "}" + "' 2>/dev/null" + NL
            import tempfile, os as _os
            with tempfile.NamedTemporaryFile(mode="w", suffix=".sh", delete=False) as f:
                f.write(scr); sp=f.name
            _os.chmod(sp, 0o755)
            r = subprocess.run(["timeout", str(timeout_sec+5), sp], timeout=timeout_sec+10, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            _os.unlink(sp)
            self._ok = (r.returncode == 0)
            if not self._ok:
                print(f"[BT-TTS] action failed (rc={r.returncode})")
        except Exception as e:
            print(f"[BT-TTS] Error: {e}")
            self._ok = False
